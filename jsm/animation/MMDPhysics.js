...
class ResourceManager {
	...
	setBasisFromArray3( t, a ) {
		let thQ = this.allocThreeQuaternion();
		let thE = this.allocThreeEuler();
		thE.set( a[0], a[1], a[2] );
...
class MMDPhysicsHelper extends Object3D {
	...
	updateMatrixWorld( force ) {
		...
		if ( this.visible ) {
			let bodies = this.physics.bodies;
			...
			for ( let i = 0, il = bodies.length; i < il; i ++ ) {
				let body = bodies[i].body;
				let child = this.children[i];

				let tr = body.getCenterOfMassTransform();
				let origin = tr.getOrigin();
				let rotation = tr.getRotation();
	...
	_init() {
		...
		function createCapsuleGeometry( radius, cylinderHeight, segmentsRadius, segmentsHeight ) {
			let geometry = new CylinderGeometry( radius, radius, cylinderHeight, segmentsRadius, segmentsHeight, true );
			let upperSphere = new Mesh( new SphereGeometry( radius, segmentsRadius, segmentsHeight, 0, Math.PI * 2, 0, Math.PI / 2 ) );
			let lowerSphere = new Mesh( new SphereGeometry( radius, segmentsRadius, segmentsHeight, 0, Math.PI * 2, Math.PI / 2, Math.PI / 2 ) );
