...
class IFCLoader extends Loader {
	...
	highlightItems( expressIds, scene, material = this.highlightMaterial ) {
		...
		expressIds.forEach((id) => {
			...
			let mesh = new Mesh(this.mapIDGeometry[id], material);
			mesh.renderOrder = 1;
			scene.add(mesh);
	...
	setVertexVisibility( geometry, index, visible ) {
		let isVisible = visible ? 0 : 1;
		let geoIndex = geometry.index.array;
	...
	setupVisibility( geometry ) {
		if (!geometry.attributes.visibility) {
		  let visible = new Float32Array(geometry.getAttribute('position').count);
	...
	getAllRelatedItemsOfType( elementID, type, relation, relatedProperty ) {
		...
		for (let i = 0; i < lines.size(); i++) {
		  	...
		  	if (foundElement) {
				let element = rel[relatedProperty];
