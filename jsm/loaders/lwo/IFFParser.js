...
IFFParser.prototype = {
	...
	parseTopForm() {
		...
		let topForm = this.reader.getIDTag();
		...
		let length = this.reader.getUint32();

		this.debugger.dataOffset = this.reader.offset;
		this.debugger.length = length;

		let type = this.reader.getIDTag();
	...
	parseSurfaceForm( length ) {
		...
		let name = this.reader.getString();
	...
	parseValueForm() {
		...
		let valueType = this.reader.getString();
	...
	parseClipLwo2( length ) {
		...
		while ( true ) {
			let tag = this.reader.getIDTag();
			let n_length = this.reader.getUint16();
			if ( tag === 'STIL' ) {
	...
	parseXVAL( type, length ) {
		let endOffset = this.reader.offset + length - 4;
		this.reader.skip( 8 );
	...
	parseLayer( length ) {
		...
		let parsedLength = 16 + stringOffset( this.currentLayer.name ); // index ( 2 ) + flags( 2 ) + pivot( 12 ) + stringlength
	...
	parseVertexMapping( length, discontinuous ) {
		let finalOffset = this.reader.offset + length;

		let channelName = this.reader.getString();
		...
		let remainingLength = length - 6 - stringOffset( name );
	...
	parseMaterialIndices( finalOffset ) {
		...
		while ( this.reader.offset < finalOffset ) {
			let polygonIndex = this.reader.getVariableLengthIndex();
			let materialIndex = this.reader.getUint16();
	...
	parseUnknownCHUNK( blockID, length ) {
		...
		let data = this.reader.getString( length );
...
DataViewReader.prototype = {
	...
	getUint16: function() {
		let value = this.dv.getUint16( this.offset );
		this.offset += 2;
		return value;
	},

	getInt32: function() {
		let value = this.dv.getInt32( this.offset, false );
		this.offset += 4;
		return value;
	},

	getUint32: function() {
		let value = this.dv.getUint32( this.offset, false );
		this.offset += 4;
		return value;
	},
	...
	getFloat32: function() {
		let value = this.dv.getFloat32( this.offset, false );
		this.offset += 4;
		return value;
	},
