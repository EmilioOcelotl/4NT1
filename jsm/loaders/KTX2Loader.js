...
class KTX2Loader extends CompressedTextureLoader {
	...
	load( url, onLoad, onProgress, onError ) {
		...
		let bufferPending = new Promise( function( resolve, reject ) {
	...
	parse( buffer, onLoad, onError ) {
		...
		let ktx = readKTX( new Uint8Array( buffer ) );
		...
		let dfd = KTX2Utils.getBasicDFD( ktx );

		KTX2Utils.createLevels( ktx, this.zstd ).then( function( levels ) {
			let basisFormat = dfd.colorModel === KTX2Model.UASTC
...
var KTX2Utils = {
	...
	createLevels: async function( ktx, zstd ) {
		...
		for ( let levelIndex = 0; levelIndex < ktx.levels.length; levelIndex ++ ) {
			let levelWidth = Math.max( 1, Math.floor( width / Math.pow( 2, levelIndex ) ) );
			let levelHeight = Math.max( 1, Math.floor( height / Math.pow( 2, levelIndex ) ) );
