	...
	parse( data ) {
		function parseASCII( data ) {
			...
			if ( colors.length !== indices.length ) {
				...
				let numTriangles = geometry.attributes.position.count / 3;
		...
		function Int32Concat( first, second ) {
			let firstLength = first.length, result = new Int32Array( firstLength + second.length );
			...
			function parseDataArray( ele, compressed ) {
				...
				if ( ele.attributes.format === 'binary' && compressed ) {
					...
					for ( var i = 0; i < dataOffsets.length - 1; i ++ ) {
						let data = fflate.unzlibSync( byteData.slice( dataOffsets[i], dataOffsets[i + 1] ) ); // eslint-disable-line no-undef
						content = data.buffer;
		...
		let meta = LoaderUtils.decodeText( new Uint8Array( data, 0, 250 ) ).split( '\n' );
