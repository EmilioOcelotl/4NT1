...
function queryKey(Components) {
  ...
  for (let n = 0; n < Components.length; n++) {
    ...
    if (typeof T === 'object') {
      let operator = T.operator === 'not' ? '!' : T.operator;
...
class SystemManager {
  ...
  registerSystem(SystemClass, attributes) {
    ...
    let system = new SystemClass(this.world, attributes);
  ...
  removeSystem(SystemClass) {
    let index = this._systems.indexOf(SystemClass);
    if (!~index) return;
  ...
  stats() {
    ...
    for (let i = 0; i < this._systems.length; i++) {
      ...
      let systemStats = (stats.systems[system.getName()] = {
        queries: {},
        executeTime: system.executeTime,
      });
      for (let name in system.ctx) {
...
class EventDispatcher {
  ...
  removeEventListener(eventName, listener) {
    let listenerArray = this._listeners[eventName];
...
class Query {
  ...
  constructor(Components, manager) {
    ...
    for (let i = 0; i < manager._entities.length; i++) {
      let entity = manager._entities[i];
...
class EntityPool extends ObjectPool {
  ...
  expand(count) {
    for (let n = 0; n < count; n++) {
      let clone = new this.T(this.entityManager);
      clone._pool = this;
      this.freeList.push(clone);
    }
    this.count += count;
...
class EntityManager {
  ...
  entityAddComponent(entity, Component, values) {
    ...
    let componentPool = this.world.componentsManager.getComponentsPool(
      Component
    );
...
this.entitiesWithComponentsToRemove.push(entity);
  ...
  _entityRemoveComponentSync(entity, Component, index) {
    ...
    entity._ComponentTypes.splice(index, 1);
    let component = entity._components[Component._typeId];
    delete entity._components[Component._typeId];
    component.dispose();
...
this.entityRemoveComponent(entity, Components[j], immediately);
  ...
  processDeferredRemoval() {
    ...
    for (let i = 0; i < this.entitiesWithComponentsToRemove.length; i++) {
      ...
      while (entity._ComponentTypesToRemove.length > 0) {
        ...
        let component = entity._componentsToRemove[Component._typeId];
  ...
  stats() {
    ...
    for (let ecsyComponentId in this.componentsManager._componentPool) {
      let pool = this.componentsManager._componentPool[ecsyComponentId];
...
class Entity {
  ...
  getComponent(Component, includeRemoved) {
    let component = this._components[Component._typeId];
  ...
  copy(src) {
    ...
    for (let ecsyComponentId in src._components) {
      let srcComponent = src._components[ecsyComponentId];
      this.addComponent(srcComponent.constructor);
      let component = this.getComponent(srcComponent.constructor);
  ...
  reset() {
    ...
    for (let ecsyComponentId in this._components) {
...
class World {
  constructor(options = {}) {
    ...
    if (hasWindow && typeof CustomEvent !== 'undefined') {
      let event = new CustomEvent('ecsy-world-created', {
...
class System {
  canExecute() {
    ...
    for (let i = 0; i < this._mandatoryQueries.length; i++) {
      let query = this._mandatoryQueries[i];
...
eventList.push(entity);
  ...
  toJSON() {
    ...
    if (this.constructor.queries) {
      let queries = this.constructor.queries;
...
function createType(typeDefinition) {
  let mandatoryProperties = ['name', 'default', 'copy', 'clone'];

  let undefinedProperties = mandatoryProperties.filter((p) => {
...
function hookConsoleAndErrors(connection) {
  let wrapFunctions = ['error', 'warning', 'log'];
  wrapFunctions.forEach((key) => {
    if (typeof console[key] === 'function') {
      let fn = console[key].bind(console);
      console[key] = (...args) => {
        connection.send({
          method: 'console',
          type: key,
