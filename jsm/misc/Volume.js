...
Volume.prototype = {
	...
	reverseAccess: function( index ) {
		...
		let y = Math.floor( ( index - z * this.yLength * this.xLength ) / this.xLength );
	...
	extractPerpendicularPlane: function( axis, RASIndex ) {
		...
		let base = [new Vector3( 1, 0, 0 ), new Vector3( 0, 1, 0 ), new Vector3( 0, 0, 1 )];
		let iDirection = [firstDirection, secondDirection, axisInIJK].find( function( x ) {
			return Math.abs( x.dot( base[0] ) ) > 0.9;
		} );
		let jDirection = [firstDirection, secondDirection, axisInIJK].find( function( x ) {
			return Math.abs( x.dot( base[1] ) ) > 0.9;
		} );
		let kDirection = [firstDirection, secondDirection, axisInIJK].find( function( x ) {
		...
		sliceAccess = function( i, j ) {
			...
			let si = ( iDirection === axisInIJK ) ? IJKIndex : ( iDirection.argVar === 'i' ? i : j );
			let sj = ( jDirection === axisInIJK ) ? IJKIndex : ( jDirection.argVar === 'i' ? i : j );
			let sk = ( kDirection === axisInIJK ) ? IJKIndex : ( kDirection.argVar === 'i' ? i : j );
